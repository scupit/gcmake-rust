...
- GCMake will warn you if assets are found outside the properly prefixed resources directory (for example, inside `resources/` instead of `resources/FULL_INCLUDE_PREFIX/`.) Ther build will still continue.
...
- resources/
  - sprite.png (Not in prefixed directory - will not be copied)
  - music.ogg (Not in prefixed directory - will not be copied)
  - FULL_INCLUDE_PREFIX/
    - config.json (Correctly placed)

### Important Files

> **Note:** Anything marked *AUTO-CONFIGURED* is automatically generated by GCMake, but should still be
> committed to version control to ensure anyone can build the project from scratch with CMake only
> (no GCMake required).
