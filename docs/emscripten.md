...
GCMake automatically handles asset preloading for Emscripten builds. Assets are packaged from the properly prefixed `resources/FULL_INCLUDE_PREFIX/` directory and mapped to the same path in Emscripten's virtual filesystem. See the [project structure](project_structure.md#resource-directory-structure) docs page for general details.

**Example Mapping:**

- Physical path: `resources/MY_PROJECT/assets/sprite.png`
- Virtual filesystem path: `/resources/MY_PROJECT/assets/sprite.png`

This preserves the prefix structure inside the virtual filesystem, preventing asset conflicts between different projects when building complex project trees.

**Library vs Executable Resource Handling:**

When using Emscripten, resources behave differently for libraries versus executables:

- **Libraries**: Emscripten compiles libraries to static libraries (`.a` files) without generating sidecar files like `.wasm`, `.data`, or `.js`. Any resources attached to a library are automatically inherited by executables that link to the library.

- **Executables**: Each Emscripten executable generates its own set of output files including `.wasm`, `.wasm.map`, and optionally `.data` (when resources are present) and `.js` (in `WITH_HTML` mode). The `.data` file contains all resources from the executable itself plus any resources inherited from linked libraries.

This means that **one `.data` file is generated per Emscripten executable output**, containing all resources from the executable and its library dependencies combined.
