...
pub fn create_project_at(
  ...
  if should_create_project {
    ...
    for nested_dir in [SRC_DIR_NAME, INCLUDE_DIR_NAME, ASSETS_DIR_NAME] {
    ...
    if !project_type_creating.is_test() {
      for nested_dir in [SUBPROJECTS_DIR_NAME, TESTS_DIR_NAME] {
        let mut extended_path = project_root.to_path_buf();
        extended_path.push(nested_dir);
        create_dir_all(extended_path)?;
      }
    ...
    for dir_requiring_include_suffix in [SRC_DIR_NAME, INCLUDE_DIR_NAME, ASSETS_DIR_NAME] {
      let mut extended_path = project_root.to_path_buf();
