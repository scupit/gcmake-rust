...
| `features` | no | Features the dependency declares: `{ name: { default, enables?, list_value? } }` (imgui's `freetype`; crow's `ssl`/`compression`). `enables` may only name features of the SAME dep. `default` may be a boolean or a `((constraint))` string (enabled by default only where the constraint holds), and `enables` entries may carry leading `((constraint))` prefixes; those constraints allow system/compiler predicates only — `feature:` predicates are a load error, because the writer turns them into plain `if()` blocks (a constrained enable edge is `list( APPEND ..._FEATURE_<name>_ENABLES ... )` appended after the unconditional `gcmake_register_feature` call) which run before feature values are resolved. Feature names gate `external_requires` entries via `(( feature:<name> ))` prefixes (validated against the dep's own declared set), and each feature is exposed to hooks as a `GCMAKE_PREDEP_<dep>_FEATURE_<name>` boolean, registered and resolved in the consumer root's generated file BEFORE any Phase A block (all shapes support this; the field also exists on the module-dep structs). Users enable them via `features:` / `use_default_features:` on the import entry (entries there may also carry system/compiler constraint prefixes), or a project feature's `enables: [dep/feature]`. Unmet feature-gated requirements become configure-time FATAL_ERROR guards instead of load errors. |
